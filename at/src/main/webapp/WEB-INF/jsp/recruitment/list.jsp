		...
		<tbody>
			<c:forEach items="${recruitments}" var="recruitment">
				<tr>
					...
					<td><a href="${recruitment.getDetailLink(job.code)}"
						class="block width-100p text-overflow-el">${recruitment.forPrintTitle}</a>
					</td>
					<td class="visible-on-sm-down"><a
						href="${recruitment.getDetailLink(job.code)}"
						class="flex flex-row-wrap flex-ai-c"> <span
							class="badge badge-primary bold margin-right-10">${recruitment.id}</span>
