		...
		<tbody>
			<c:forEach items="${artworks}" var="artwork">
				<tr>
					...
					<td><a href="${artwork.getDetailLink()}"
						class="block width-100p text-overflow-el">${artwork.forPrintTitle}</a>
					</td>
					<td class="visible-on-sm-down"><a
						href="${artwork.getDetailLink()}"
						class="flex flex-row-wrap flex-ai-c"> <span
							class="badge badge-primary bold margin-right-10">${artwork.id}</span>
